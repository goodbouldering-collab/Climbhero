...
- ✅ **横フリップ動画カルーセル** 🎬⚡ **NEW**
  - **コンパクトデザイン**: 縦長表示を解消し、他コンテンツの視認性向上
  - **横フリップアニメーション**: 次/前の動画へスムーズに3D回転遷移（250msアニメーション）
  - **左右スワイプナビゲーション**: 左右の矢印ボタンで前後の動画に移動可能
  - **ドットインジケーター**: 現在位置を視覚化（最大10個表示、スクロール対応）
  - **自動連続再生**: 15秒ごとに自動で次の動画へ（ループ再生）
  - **再生/停止コントロール**: ワンタップで自動再生のON/OFF切り替え
  - **プログレスバー**: 上部に赤→ピンクのグラデーションで進行状況表示
  - **動画情報オーバーレイ**: タイトル、視聴回数、いいね数、プラットフォーム、進行カウント（例: 3/20）
